...
if ($c_post_content_result || mysql_num_rows($c_post_content_result) > 0) {
    ...
    while ($c_content_puller = mysql_fetch_assoc($c_post_content_result)) {
        ...
        $query = "SELECT * FROM `main_team` WHERE sport_id = '$_id'  ";
        ...
        if ($result || mysql_num_rows($result) > 0) {
            ...
            while ($puller = mysql_fetch_assoc($result)) {
               ...
               echo $f_name = $puller['f_name'];
               echo  $l_name= $puller['last_name'];
               echo  $email = $puller['email'];

                echo '<tr><td>$email</td> <td> $f_name </td> <td> $l_name </td> </tr> ';
