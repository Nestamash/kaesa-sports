...
if (isset ($_SESSION['username']) && !empty ($_SESSION['username'])){
		...
		$c_sql_query= "SELECT * FROM `coach` WHERE coach_username = '$_pu'  ";
									...
									while($c_content_puller=mysql_fetch_assoc($c_post_content_result)){
											...
											$_CoachFName = $c_content_puller['coach_f_name'];
											$_TopCoachProfilePicName = $c_content_puller['coach_profile_pic'];
											$_TopCoachProfilePicExt = $c_content_puller['coach_profile_pic_ext'];
...
?>
	...
	<!-- Main -->
		<div id='page'>
					<h1 align='center'> <?php echo"$_SportName";?> COACH'S BOARD</h1>
			<!-- Extra -->
				...
				<div id='card' class='container'>
					<a href=''><img src='http://localhost/kaesa/login/assets/API/uploads/<?php echo"$_CoachProfilePicName";?>' id="p-pic" class='profile_pic'  alt='Fred' title='<?php echo"$_CoachFName";?>'></a>
						...
						while($comment_puller=mysql_fetch_assoc($post_comment_result)){
									...
									$_CUsername = $comment_puller['player_username'];
							...
							<!--Form, Message Back-->
							<div>
							<form method='post' id="styled-form" action ='assets/API/CoachCommentPost.php'>
							<h4>Leave a Comment here</h4>
							<input type='hidden' name='player_username' value='Coach <?php echo"$_CoachFName ";?>'>
							<input type='hidden' name='sport_id' value='<?php echo"$_si";?>'>
							<input type='hidden' name='post_id' value='<?php echo"$_PostId ";?>'>
							<input type='hidden' name='comment_type' value='coach-comment'>
							<textarea name='comment_content' required id='styled' maxlength='150' placeholder='<?php echo"$_CoachFName, ";?> Comment as Coach Here...'></textarea>
	...
	<br><br><br>
				...
				<h4>ADD A POST</h4>
				<p>To add a Post, Use the NEW POST button on the Navigation </p>
				</div>
				</div>
				</div>
	<!-- Copyright -->
